...
const spinSlotMachine = () => {
  const duration = 10 // Duración total de 10 segundos
  const itemHeight = 60 // Altura de cada nombre
  const totalDistance = names.value.length * itemHeight * 4 // Movemos el contenedor para dar sensación de velocidad
  const winnerIndex = Math.floor(Math.random() * names.value.length) // Selecciona el índice ganador

  gsap.to(namesContainer.value, {
    y: -totalDistance, // Mueve hacia arriba rápidamente
    duration: duration, // Duración total
    ease: "power2.out", // Comienza rápido y desacelera suavemente
    onComplete: () => {
      gsap.to(namesContainer.value, {
        y: -(winnerIndex * itemHeight), // Ajusta al ganador
        duration: 1, // Movimiento suave al final
        ease: "power2.out",
        onComplete: () => {
          gsap.to(namesContainer.value.children[winnerIndex], {
            scale: 1.5,
            duration: 0.3, // Efecto de agrandamiento para el ganador
            ease: "power2.out",
            onComplete: () => {
              gsap.to(namesContainer.value.children[winnerIndex], {
                scale: 1,
