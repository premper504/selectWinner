...
const prepareDisplayNames = () => {
  // Hacemos el loop infinito duplicando varias veces la lista de nombres
  displayNames.value = [...names.value, ...names.value, ...names.value]
}

const spinSlotMachine = () => {
  const duration = 12 // 12 segundos para dar más tiempo a la desaceleración
  const itemHeight = 60 // Altura de cada nombre
  const totalDistance = names.value.length * itemHeight * 2 // Multiplicamos la distancia total para hacer el loop más largo
  const winnerIndex = Math.floor(Math.random() * names.value.length)

  gsap.to(namesContainer.value, {
    y: -totalDistance, // Desplazamos mucho más para dar la sensación de loop
    duration: duration,
    ease: "power2.out", // Hacemos que la desaceleración dure más tiempo
    onComplete: () => {
      // Reseteamos la posición y ajustamos al ganador
      gsap.set(namesContainer.value, { y: 0 })
      gsap.to(namesContainer.value, {
        y: -(winnerIndex * itemHeight),
        duration: 1, // Ajustamos el tiempo de rebote del ganador
        ease: "bounce.out",
        onComplete: () => {
