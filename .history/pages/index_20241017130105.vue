...
const prepareDisplayNames = () => {
  displayNames.value = [...names.value, ...names.value] // Duplicar los nombres para asegurar una animación continua
}

const spinSlotMachine = () => {
  const totalDuration = 10 // Duración total de 10 segundos
  const itemHeight = 60 // Altura de cada nombre
  const winnerIndex = Math.floor(Math.random() * names.value.length)

  const totalDistance = (names.value.length * 5) * itemHeight // Aumentamos la distancia para dar efecto de velocidad

  gsap.to(namesContainer.value, {
    y: -totalDistance, // Desplazamos lo suficiente para el efecto rápido
    duration: totalDuration, // Duración total de 10 segundos
    ease: "power2.out", // Empieza rápido y desacelera de manera continua
    onComplete: () => {
      // Ajustamos al nombre del ganador al final de la animación
      gsap.to(namesContainer.value, {
        y: -(winnerIndex * itemHeight),
        duration: 1, // Ajuste final suave al ganador
        ease: "bounce.out", // Efecto rebote para el ganador
        onComplete: () => {
