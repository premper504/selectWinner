...
const startShuffle = (namesContainerElement) => {
  ...
  const duration = 15; // Duración total de la animación para que sea más suave

  // Creamos un timeline de GSAP para el desplazamiento inicial
  ...
  shuffleTimeline.to(namesContainerElement, {
    ...
    onComplete: () => {
      // Al completar, elegimos el ganador final de manera aleatoria
      ...
      gsap.to(namesContainerElement, {
        ...
        duration: 3,  // Lo hacemos más lento para darle más impacto
...
const getGenioData = async (namesContainerElement) => {
  try {
    const { data, error } = await $supabase
      ...
      .limit(300) // Limitar a 300 registros
