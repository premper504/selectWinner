...
const prepareDisplayNames = () => {
  displayNames.value = [...names.value, ...names.value] // Duplicar los nombres para dar un efecto fluido
}

const spinSlotMachine = () => {
  const totalDuration = 10 // Duración total de la animación (10 segundos)
  const itemHeight = 60 // Altura de cada nombre
  const totalDistance = names.value.length * itemHeight * 4 // Recorremos suficiente para darle el efecto de velocidad
  const winnerIndex = Math.floor(Math.random() * names.value.length)

  gsap.to(namesContainer.value, {
    y: -totalDistance, // Movemos el contenedor hacia arriba rápidamente
    duration: totalDuration, // Duración total de la animación
    ease: "power2.inOut", // Empieza rápido, desacelera gradualmente
    onComplete: () => {
      // Ajustamos al nombre del ganador al final de la animación
      gsap.to(namesContainer.value, {
        y: -(winnerIndex * itemHeight),
        duration: 1, // Ajuste final suave
        ease: "power3.out", // Suave desaceleración
        onComplete: () => {
...
.name-item {
  ...
  justify-content: center;
  align-items: center;
  white-space: nowrap; /* Para evitar salto de línea */
  overflow: hidden;
