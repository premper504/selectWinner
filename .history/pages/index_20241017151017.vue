...
const startSelection = () => {
  if (isSelecting.value) return; // Evita que se ejecute si ya está en curso
  isSelecting.value = true; // Inicia el proceso de selección
  showResult.value = true;

  // Asegúrate de ejecutar la animación de la imagen
  setTimeout(() => {
    animateHeaderGenio();
  }, 500); // Añadimos un pequeño retraso para que sea visible la transición

  spinSlotMachine();
}

const animateHeaderGenio = () => {
  if (headerGenio.value) {
    gsap.fromTo(headerGenio.value,
      { scale: 0, opacity: 0 },  // Empieza pequeño y transparente
      { scale: 1, opacity: 1, duration: 1.5, ease: "power2.out" } // Crece a tamaño normal
    );
  }
}
      ...
      gsap.to(namesContainer.value, {
        ...
        onComplete: async () => {
          ...
          try {
            ...
            gsap.to(namesContainer.value.children[winnerIndex], {
              ...
              onComplete: () => {
                gsap.to(namesContainer.value.children[winnerIndex], {
                  ...
                  onComplete: () => {
                    showConfetti();
                    isSelecting.value = false; // Restablecemos la bandera
                  }
                });
              }
            });
            ...
            isSelecting.value = false; // Restablecemos la bandera en caso de error
          }
        }
      });
    }
...
const showConfetti = () => {
  if (!showCongrats.value) { // Aseguramos que solo se muestre una vez
    createConfetti();
