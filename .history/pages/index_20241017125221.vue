...
const prepareDisplayNames = () => {
  displayNames.value = [...names.value, ...names.value] // Nombres duplicados para hacer la animación fluida
}

const spinSlotMachine = () => {
  const duration = 8 // Duración total de la animación
  const itemHeight = 60 // Altura de cada nombre
  const totalDistance = names.value.length * itemHeight * 5 // Recorrido extendido para mayor efecto

  const winnerIndex = Math.floor(Math.random() * names.value.length)

  gsap.to(namesContainer.value, {
    y: -totalDistance, // Desplazamos en toda la distancia
    duration: duration, // Tiempo total de la animación
    ease: "power2.inOut", // Solo una transición suave de rápido a lento
    onComplete: () => {
      // Al final, ajustamos al ganador
      gsap.to(namesContainer.value, {
        y: -(winnerIndex * itemHeight),
        duration: 1, // Suave ajuste final al ganador
