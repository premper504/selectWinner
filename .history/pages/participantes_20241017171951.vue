<template>
    <div class="main-body">
      ...
      <div class="maxwidth container">
        ...
        <div v-infinite-scroll="loadMore" infinite-scroll-disabled="isLoading || winners.length >= totalRecords" infinite-scroll-distance="10">
          <el-table
  ...
  if (isLoading.value) {
    console.log('Ya está cargando, no hacer nada.')
    return
  }

  if (winners.value.length >= totalRecords.value) {
  ...
  try {
    // Añadimos una impresión para verificar la consulta que estamos haciendo
    console.log(`Obteniendo datos de la página ${page.value}`)
    ...
    if (error) {
      ...
      if (data.length === 0) {
        console.log('No se encontraron registros en esta página.')
      } else {
        console.log('Datos obtenidos:', data)  // Verificar los datos en la consola
