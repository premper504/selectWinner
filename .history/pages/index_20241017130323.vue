...
const spinSlotMachine = () => {
  const totalDuration = 10 // Duración total aumentada para más suspenso
  ...
  gsap.to(namesContainer.value, {
    y: -totalDistance,
    duration: totalDuration * 0.7, // 70% de la duración total para el desplazamiento rápido
    ease: "power2.out",
    onComplete: () => {
      // Fase lenta para añadir suspenso
      gsap.to(namesContainer.value, {
        y: -(winnerIndex * itemHeight),
        duration: totalDuration * 0.3, // 30% de la duración total para la fase lenta
        ease: "slow(0.7, 0.7, false)", // Transición suave para aumentar el suspenso
        onComplete: () => {
